...
Max72xxPanel2::Max72xxPanel2(byte csPin, byte hDisplays, byte vDisplays) : Adafruit_GFX(hDisplays << 3, vDisplays << 3) {

  Max72xxPanel2::SPI_CS = csPin;

  byte displays = hDisplays * vDisplays;
  Max72xxPanel2::hDisplays = hDisplays;
  Max72xxPanel2::bitmapSize = displays << 3;

  Max72xxPanel2::bitmap = (byte*)malloc(bitmapSize);
  Max72xxPanel2::matrixRotation = (byte*)malloc(displays);
  Max72xxPanel2::matrixPosition = (byte*)malloc(displays);
...
// BEGINN NEUE FUNKTION VON DIMITRI

byte Max72xxPanel2::readByte(uint16_t xx, uint16_t yy){
	bool state;
	int16_t x = xx;
	byte y = yy;
	byte tmp;
	...
	// Read the color bit in our bitmap buffer.

	byte *ptr = bitmap + x + WIDTH * (y >> 3);
	byte val = 1 << (y & 0b111);

	// if ( color ) {				// wie kann man hier die pixel auslesen anstatt beschreiben????
	// 	*ptr |= val;
	// }
	// else {
	// 	*ptr &= ~val;
...
// ENDE NEUE FUNKTION DIMITRI

void Max72xxPanel2::setPosition(byte display, byte x, byte y) {
	matrixPosition[x + hDisplays * y] = display;
}

void Max72xxPanel2::setRotation(byte display, byte rotation) {
	matrixRotation[display] = rotation;
}

void Max72xxPanel2::setRotation(uint8_t rotation) {
	Adafruit_GFX::setRotation(rotation);
}

void Max72xxPanel2::shutdown(boolean b) {
  spiTransfer(OP_SHUTDOWN, b ? 0 : 1);
}

void Max72xxPanel2::setIntensity(byte intensity) {
  spiTransfer(OP_INTENSITY, intensity);
}

void Max72xxPanel2::fillScreen(uint16_t color) {
  memset(bitmap, color ? 0xff : 0, bitmapSize);
}

void Max72xxPanel2::drawPixel(int16_t xx, int16_t yy, uint16_t color) {
...
void Max72xxPanel2::spiTransfer(byte opcode, byte data) {
